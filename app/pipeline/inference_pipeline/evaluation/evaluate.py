...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate the inference pipeline without relying on Opik."
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
...
def resolve_dataset_path(dataset_name: str, dataset_path: Path | None) -> Path:
    ...
    raise FileNotFoundError(
        f"Could not find dataset for name '{dataset_name}'. "
        "Provide --dataset_path pointing to a JSON file generated by the dataset pipeline."
    )
...
def run_evaluation(
    ...
    return {
        metric_name: fmean(values) if values else 0.0
